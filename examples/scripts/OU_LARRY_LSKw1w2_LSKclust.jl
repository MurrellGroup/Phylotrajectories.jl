...
well = "LSKw1w2"
path = "/home/masha/CLEAN_PhyloTrajectory_paper/results/LARRY/$(well)"

clono_info, cluster_names, _,count_matrix = import_count_matrix(
    "/home/masha/PhyloTraj_paper/Final_notebooks/LARRY/LSK_well_1_2_Clustering_LSK.csv",
    :Clone_id,
    :Cluster_res15_Ann,
    :Cell_barcode,
    cluster_filters = ["Cluster_18", "Cluster_19", "Cluster_21", "Cluster_22", "Cluster_23"]  #{27, 29, 31, 32, 33, 34}
);
    ...
    return OUContinuousModel(
        update = OUContinuousUpdate(
            ...
            root_sampler = OUGaussianStateSample(MvNormal(zeros(2), Diagonal([0.01, 0.01])), 
                            ...
                            1e-3, 1),
            ou_eqmu_sampler = OUEqmuSampler(Normal(0.0, 0.5), Normal(1.5, 1.0), 1.0, 0.1),
...
savefig(combined_plt, "$(path)/OU_LARRY_LSKclust_$(well).pdf")
savefig(hipster_tree, "$(path)/OU_LARRY_LSKclust_$(well)_hipster.pdf")

# Save a representative tree from the MCMC samples (e.g., the last tree)
Phylo.write("$(path)/OU_LARRY_LSKclust_$(well)_HIPSTR.newick", get_phylo_tree(hip))
...
savefig(pl_cred, "$(path)/OU_LARRY_LSKclust_$(well)_$(digamma_value)_$(trigamma_value)_HIPSTR_with_CRED_seed_$(number_seed).pdf")
...
run_ou_and_build_clone_matrix(trees,
    ...
    0.5,
    0.5,
    "$(path)/OU_LARRY_LSKclust_$(well)_clone_matrix_seed_$(number_seed).csv"
    )