...
clono_info, cluster_names, _,count_matrix = import_count_matrix(
    "/home/masha/PhyloTraj_paper/Final_notebooks/LARRY/LK_well_1_2_Clustering_LK.csv",
    :Clone_id,
    :Cluster_res20_Ann,
    :Cell_barcode,
    cluster_filters = ["Cluster_24", "Cluster_25"]  #{24, 25}
);
...
savefig(combined_plt, "$(path)/OU_LARRY_LKclust_$(well).pdf")
savefig(hipster_tree, "$(path)/OU_LARRY_LKclust_$(well)_hipster.pdf")

# Save a representative tree from the MCMC samples (e.g., the last tree)
Phylo.write("$(path)/OU_LARRY_LKclust_$(well)_HIPSTR.newick", get_phylo_tree(hip))
...
savefig(pl_cred, "$(path)/OU_LARRY_LKclust_$(well)_$(digamma_value)_$(trigamma_value)_HIPSTR_with_CRED_seed_$(number_seed).pdf")
...
run_ou_and_build_clone_matrix(trees,
    ...
    0.5,
    0.5,
    "$(path)/OU_LARRY_LKclust_$(well)_clone_matrix_seed_$(number_seed).csv"
    )
