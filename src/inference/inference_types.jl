...
- `n_random_trees::Int=1`: the number of initial tree samples.
"""
struct DiscreteModel <: InferenceModel
    ...
    function DiscreteModel(;
        ...
        new(jump, a, b, Ne, sample_rate, start_branch_length, max_cycles, n_random_trees)
    end
end
...
- `sample_rate::Float64=10.0`: the initial tree's sample rate.
- `start_branch_length::Float64=0.1`: the initial tree's non-root branch lengths.
- `samples::Int=10`: number of MCMC samples to collect after burn-in, with automatic spacing to reduce autocorrelation.
